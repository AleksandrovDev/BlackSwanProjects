...
class Handler(Extract, GetPages):
    ...
    def getpages(self, searchquery):
        result = []
        link_1 = f"https://www.nse.or.jp/e/meigara/stocksch/list.html?schOrder_Target=stockCode&schOrder_Rule=ASC&schList_Count=10&schKey_Code=&schKey_name_j={searchquery}&schKey_Industry=&schKey_Month=&schKey_Unit=&page=1"
        link_2 = f"https://www.nse.or.jp/api/stock/search.json?stockCode=&stockName_e={searchquery}&listedSingle=null&industryCode=&listedClose=&tradingUnit=&dispType=stockCode&dispOrder=ASC&dispCount=10&dispPage=1"

        self.get_working_tree_api(link_2, "api")
        for i in self.api["stock"]:
            temp = {
                "name": i["stockName_e"],
                "ind": i["industryName_e"],
                "url": i["stockUrl_e"],
                "ident": i["stockCode"],
                "link": link_2,
            }
    ...
    def get_address(
        ...
        if addr:
            ...
            try:
                temp["city"] = splittedAddr[-1].split(" ")[-1]
            except:
                pass
            try:
                ...
                pass
            try:
                temp["fullAddress"] = " ".join(splittedAddr) + ", Switzerland"
            except:
                pass
            try:
                patterns = ["Suite\s\d+"]
    ...
    def fillField(
        self, fieldName, key=None, xpath=None, test=False, reformatDate=None, el=None
    ):
        if xpath:
        ...
        if el:
            ...
            if fieldName == "hasActivityStatus":
                statuses = {
                    "Aktivmitglied": "active member",
                    "Partnermitglied": "partner member",
                    "Einzelmitglied": "single member",
                    "Qualified independent wealth manager VSV": "Qualified independent wealth manager VSV",
                }
                self.overview[fieldName] = statuses[el]
    ...
    def get_overview(self, link_name):
        ...
        if type(link_name) == dict:
            ...
            link_name = json.loads(link_name)

        try:
            self.overview["vcard:organization-name"] = link_name["name"]

        except:
            return None
        self.overview["isDomiciledIn"] = "JP"
        self.overview["@source-id"] = self.NICK_NAME
        self.overview["bst:sourceLinks"] = [link_name["link"]]
        try:
            if link_name["url"]:
                self.overview["hasURL"] = link_name["url"]
        except:
            pass

        try:
            self.overview["bst:businessClassifier"] = [
                {"code": "", "description": link_name["ind"], "label": ""}
            ]
        except:
            pass
        ...
        try:
            self.overview["identifiers"] = {
                "other_company_id_number": link_name["ident"]
            }
        except:
            pass
        ...
        self.overview["bst:stock_info"] = {
            "mic_code": "XNGO",
            "ticket_symbol": "",
            "main_exchange": "Nagoya Stock Exchange",
        }
