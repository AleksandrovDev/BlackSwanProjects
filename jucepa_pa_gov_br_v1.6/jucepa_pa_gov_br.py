...
class Handler(Extract, GetPages):
    ...
    def get_file2(
        self, xpath, url, headers=None, data=None, method="GET", file_base_url=None
    ):
        fd = None
        try:
            if url is not None:
                tree = self.get_tree(
                    url, headers=headers, data=data, method=method, verify=False
                )
                file_url = file_base_url + tree.xpath(xpath)[0].attrib["href"]
            else:
                file_url = file_base_url
            file_url = file_url.replace(" ", "%20")
            fd = requests.get(file_url, verify=False)
            fd = BytesIO(fd.content)
        except Exception as e:
            pass
        return fd
    ...
    def getpages(self, searchquery):
        result = []
        df = self.getpages_pdf2(
            searchquery,
            search_column=1,
            file_base_url="https://www.jucepa.pa.gov.br/sites/default/files/anexos/EmpPubl_e_SocEconMista_PA.pdf",
            pages="1-2",
            multiple_tables=True,
        )
        while not df:
        ...
        count = 0
        for page in df:
            names = page.iloc[:, 1].astype(str).str.replace("\r", " ")
            jurisdictions = page.iloc[:, 0].astype(str).str.replace("\r", " ")

            cities = page.iloc[:, 7].astype(str).str.replace("\r", " ")
            add2 = page.iloc[:, 6].astype(str).str.replace("\r", " ")
            add1 = page.iloc[:, 4].astype(str).str.replace("\r", " ")
            vat = page.iloc[:, 3].astype(str).str.replace("\r", " ")
            other = page.iloc[:, 2].astype(str).str.replace("\r", " ")

            for name, jur, city, a1, a2, v, ot in zip(
                names, jurisdictions, cities, add1, add2, vat, other
            ):
                name = (
                    str(name)
                    .replace("('", "")
                    ...
                    .replace("   ", "")
                    .replace("  ", "")
                    .replace("COM PANHI ADE", "COMPANHIA DE")
                    .replace("DAAM ", "DA AM")
                )
                br = re.findall("\d+", jur)
                if not br:
                    ...
                    temp[company] = {
                        ...
                        "mdaas:RegisteredAddress": {
                            "city": city,
                            "fullAddress": a1 + ", " + a2 + ", " + city,
                        },
                        "identifiers": {
                            "vat_tax_number": v,
                            "other_company_id_number": ot.replace(" ", ""),
                        },
                    }
                else:
                    temp[company]["bst:businessClassifier"].append(
                        {"code": jur, "description": name, "label": ""}
                    )

        for i in list(temp.keys()):
            if searchquery.lower() in i.lower():
                result.append({i: temp[i]})

            count += 1

        result = [json.dumps(i) for i in result]
    ...
    def get_overview(self, link):
        ...
        try:
            cname = list(link.keys())[0]
            res["vcard:organization-name"] = cname
        except:
            return {}

        try:
            res.update(link[cname])
        except:
            pass

        res["isDomiciledIn"] = "BR"
        res["hasActivityStatus"] = "Active"
        res["@source-id"] = "jucepa_pa_gov_br"
        res["sourceDate"] = datetime.datetime.today().strftime("%Y-%m-%d")
    ...
    def get_branches(self, link):
        ...
        for page in df[1:]:
            names = page.iloc[:, 1].astype(str).str.replace("\r", " ")
            sourceIds = page.iloc[:, 2].astype(str).str.replace("\r", " ")
            addr1 = page.iloc[:, 4].astype(str).str.replace("\r", " ")
            addr2 = page.iloc[:, 6].astype(str).str.replace("\r", " ")
            cities = page.iloc[:, 7].astype(str).str.replace("\r", " ")
        for n, sid, a1, a2, c in zip(names, sourceIds, addr1, addr2, cities):
            n = str(n).replace("   ", "").replace("  ", "").replace("DAAM ", "DA AM")
            a1 = "" if a1 == "nan" else a1
            a2 = "" if a2 == "." else a2.replace("   ", "").replace("  ", "")
            if n == cname:
                temp = {}
                temp["vcard:organization-name"] = n
                temp["@sourceReferenceID"] = sid.replace(" ", "")
                ...
                if c:
                    fullAddr.append(c)
                if fullAddr:
                    fullAddr = ", ".join(fullAddr)

                temp["mdaas:RegisteredAddress"] = {"city": c, "fullAddress": fullAddr}
                temp["isDomiciledIn"] = "BR"
                temp["entity_type"] = "Q"
