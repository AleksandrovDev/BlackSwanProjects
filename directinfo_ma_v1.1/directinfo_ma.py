...
class Handler(Extract, GetPages):
    ...
    def getpages(self, searchquery):
        ...
        companies = []
        for company in self.extractedData[0]:
            companies.append(company["denomination"])
        return companies
    ...
    def get_overview(self, link):
        ...
        self.get_initial_page(link)
        companyInformation = self.extractedData[0][0]
        companyId = companyInformation["id"]

        link2 = (
            f"https://www.directinfo.ma/directinfo-backend/api/entreprise/{companyId}"
        )
        requestOptions = {
            "url": link2,
        }
        ...
        hardcodedFields = {
            ...
            "isDomiciledIn": "MA",
            "bst:registryURI": self.base_url
            + "fiche-detail/"
            + str(companyInformation["id"])
            + "/"
            + companyInformation["denomination"].replace(" ", "-"),
            "bst:sourceLinks": [
                self.base_url
                + "fiche-detail/"
                + str(companyInformation["id"])
