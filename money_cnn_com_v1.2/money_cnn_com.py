...
class Handler(Extract, GetPages):
    base_url = "https://edition.cnn.com/business/"
    ...
    def getpages(self, searchquery):
        ...
        companies = [i.get("s") for i in self.extractedData[0]]
        companies = [
            f"https://money.cnn.com/quote/quote.html?symb={i}" for i in companies
        ]

        return companies

    def get_initial_page(self, searchquery, add=None):
        link = f"https://edition.cnn.com/business"
        ...
        link = f"https://markets.money.cnn.com/common/symbolLookup/getSymbols.asp?jsoncallback=symbolSearch&q={searchquery}&render=JSON&callback=symbolSearch"
    ...
    def get_overview(self, link):
        ...
        fetchedFields = {
            "vcard:organization-name": ['//h1[@class="wsod_fLeft"]/text()'],
        }
        ...
        try:
            ...
            fetchedFieldsPro = {
                "isDomiciledIn": [
                    '//td[@class="wsod_companyAddress"]/div[1]/div[3]/text()',
                    handleDomicled,
                ],
                "bst:businessClassifier": {
                    "code": [""],
                    "description": [
                        '//text()[contains(., "INDUSTRY")]/../div[1]/text()'
                    ],
                    "label": [""],
                },
                "mdaas:RegisteredAddress": {
                    "zip": [
                        '//td[@class="wsod_companyAddress"]/div[1]/div[2]/text()',
                        handleZip,
                    ],
                    "country": [
                        '//td[@class="wsod_companyAddress"]/div[1]/div[3]/text()',
                        self.get_country_from_address,
                    ],
                    "streetAddress": [
                        '//td[@class="wsod_companyAddress"]/div[1]/div[2]/text()',
                ...
                "hasLatestOrganizationFoundedDate": [
                    '//div[@id="wsod_companyDescription"]//text()',
                    handleFounded,
                ],
                "bst:description": ['//div[@id="wsod_companyDescription"]//text()'],
                "registeredIn": [
                    '//td[@class="wsod_companyAddress"]/div[1]/div[3]/text()',
                    lambda x: x.split(" ")[1] if x else None,
                ],
                "tr-org:hasRegisteredPhoneNumber": [
                    '//div[@class="wsod_companyContactInfo wsod_companyPhoneURL"]/div[1]/text()'
                ],
                "bst:stock_info": {
                    "mic_code": [""],
                    "ticket_symbol": [
                        '//span[@class="wsod_smallSubHeading"]/text()',
                        lambda x: x[1:-1] if x else None,
                    ],
                    "main_exchange": [""],
                },
                "Service": {
                    "serviceType": ['//text()[contains(., "SECTOR")]/../div[1]/text()'],
                },
                "hasURL": [
                    '//div[@class="wsod_companyContactInfo wsod_companyPhoneURL"]/div[3]/a/@href'
                ],
            }

            profileResult = self.extract_data(
                fetchedFieldsPro, hardcodedFields, self.extractedData
            )
            if profileResult["bst:businessClassifier"].get("description"):
                profileResult["bst:businessClassifier"] = [
                    profileResult["bst:businessClassifier"]
                ]

            result.update(profileResult)
        except:
            pass
