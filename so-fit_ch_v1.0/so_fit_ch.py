...
class Handler(Extract, GetPages):
    ...
    def getpages(self, searchquery):
        ...
        df = self.getpages_pdf(
            pages="all",
            file_base_url="https://so-fit.ch/wp-content/uploads/2021/02/Liste-societes-daudit-27.01.2022.pdf",
        ...
        for page in df:
            addresses = page.iloc[:, 3].str.replace("\r", " ")
            addresses2 = page.iloc[:, 6].str.replace("\r", " ")

            names = page.iloc[:, 1].str.replace("\r", " ")

            mails = page.iloc[:, 9].str.replace("\r", " ")
            phones = page.iloc[:, 8].str.replace("\r", " ")
            registered = page.iloc[:, 7].str.replace("\r", " ")

            for name, addr, addr2, mail, phone, reg in zip(
                names, addresses, addresses2, mails, phones, registered
            ):
                if searchquery.lower() in name.lower():
                    result.append(f"{name}=?{addr}=?{addr2}=?{mail}=?{phone}=?{reg}")
    ...
    def get_overview(self, link_name):
        ...
        self.overview["bst:email"] = link_name.split("=?")[3]
        self.overview["isDomiciledIn"] = "CH"
        self.overview["registeredIn"] = link_name.split("=?")[-1]
        self.overview["tr-org:hasRegisteredPhoneNumber"] = link_name.split("=?")[-2]
        ...
        try:
            self.overview["mdaas:RegisteredAddress"]["fullAddress"] = (
                link_name.split("=?")[1] + " ," + link_name.split("=?")[2]
            )
        except:
            pass
        ...
        self.overview[
            "regulator_name"
        ] = "Organisme d'Autorégulation des Gérants de Patrimoine (OARG SRO)"
        self.overview["regulatorAddress"] = {
            "fullAddress": "SO-FIT 2, Rue Pedro-Meylan 1208 Geneva",
            "city": "Geneva",
            "country": "Switzerland",
        }
