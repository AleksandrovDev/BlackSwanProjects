...
class Handler(Extract, GetPages):
    base_url = "https://eservices.secp.gov.pk/"
    ...
    def getpages(self, searchquery):
        extractedPage = self.get_initial_page(searchquery)
        companies = self.get_companies(extractedPage)
        return companies

    def get_companies(self, extractedPage):
        companies = extractedPage.xpath(
            '//div[@class="webfx-columnlist-body"]//tr/td[2]/text()'
        )
        return companies

    def get_initial_page(self, searchquery):
        link = f"https://eservices.secp.gov.pk/eServices/NameSearch.jsp"
        requestOptions = {"url": link, "method": "GET", "returnType": "tree"}
        self.getDataFromPage(requestOptions)

        link = f"https://eservices.secp.gov.pk/eServices/ControllerServlet"
        data = {
            "request_id": "SEARCH_NAME",
            "requesterProcess": "",
    ...
    def get_result_list_by_path(self, pathToResultList):
        ...
        if outputType == "api":
            ...
            print(pathToResultList)
            print(self.get_dict_value_by_path(pathToResultList, self.extractedData))
    ...
    def get_overview(self, link):
        extractedPage = self.get_initial_page(link)

        companyInformation = self.find_company_on_the_page(
            f'//div[@class="webfx-columnlist-body"]//tr/td[2]/text()[contains(.,"{link}")]/../..'
        )
        ...
        def extraHandlingIsIncorporatedIn(date):
            if date:
                date = date.split(" ")[0]
                try:
                    date = self.reformat_date(date, "%d-%m-%Y")
                except:
                    date = self.reformat_date(date, "%d/%m/%Y")
                return date
            return ""
        ...
        fetchedFields = {
            "hasActivityStatus": ["./td[3]/text()"],
            "isIncorporatedIn": ["./td[6]/text()", extraHandlingIsIncorporatedIn],
            "identifiers": {"trade_register_number": ["./td[5]/text()"]},
            "bst:registrationId": ["./td[5]/text()"],
        }
