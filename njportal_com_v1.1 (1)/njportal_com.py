...
class Handler(Extract, GetPages):
    ...
    def get_overview(self, link):
        ...
        result = {}
        try:
            result["vcard:organization-name"] = link["0"][0]
        except:
            return {}
        result["isDomiciledIn"] = "US"
        try:
            result["identifiers"] = {"other_company_id_number": link["3"][0]}
            result["bst:registrationId"] = link["3"][0]
        except:
            pass

        try:
            result["lei:legalForm"] = {"code": "", "label": link["4"][1]}
        except:
            pass

        try:
            result["isIncorporatedIn"] = self.reformat_date(link["2"][0], "%m/%d/%Y")
        except:
            pass

        try:
            result["mdaas:RegisteredAddress"] = {
                "country": "US",
                "city": link["1"][0],
                "fullAddress": link["1"][0] + ", US",
            }
        except:
            pass
