...
class Handler(Extract, GetPages):
    ...
    def getpages(self, searchquery):
        ...
        df = self.getpages_pdf(
            pages="all",
            file_base_url="https://acpr.banque-france.fr/sites/default/files/20210105_liste_organismes_actifs_0.pdf",
        ...
        for page in df:
            ...
            names = page.iloc[:, 2].str.replace("\r", " ")
            registered = page.iloc[:, 6].str.replace("\r", " ")
            busClas = page.iloc[:, 3].str.replace("\r", " ")
            lei = page.iloc[:, 0].str.replace("\r", " ")
            regID = page.iloc[:, 1].astype(str).str.replace("\r", " ")
            other = page.iloc[:, 7].astype(str).str.replace("\r", " ")

            for name, addr, regIN, bclass, l, regI, ot in zip(
                names, addresses, registered, busClas, lei, regID, other
            ):
                if searchquery.lower() in name.lower():
                    result.append(
                        f"{name}=?{addr}=?{regIN}=?{bclass}=?{l}=?{regI}=?{ot}"
                    )
    ...
    def get_address(
        ...
        if addr:
            ...
            temp = {"fullAddress": addr, "country": "France"}
            if zipPattern:
    ...
    def get_overview(self, link_name):
        ...
        self.overview["regulator_name"] = "Autorité de Controle Prudentiel"
        self.overview[
            "regulator_url"
        ] = "https://acpr.banque-france.fr/en/authorisation/register-insurers"
        self.overview["RegulationStatus"] = "Authorised"
        self.overview["isDomiciledIn"] = "FR"
        self.overview["bst:sourceLinks"] = [
            "https://acpr.banque-france.fr/sites/default/files/20210105_liste_organismes_actifs_0.pdf"
        ]
        self.overview["registeredIn"] = link_name.split("=?")[2]
        self.overview["bst:businessClassifier"] = [
            {"code": "", "description": link_name.split("=?")[3], "label": ""}
        ]
        if link_name.split("=?")[5] != "nan":
            self.overview["bst:registrationId"] = link_name.split("=?")[5]

        if link_name.split("=?")[4] != "nan":
            self.overview["identifiers"] = {
                "legal_entity_identifier": link_name.split("=?")[4],
        ...
        self.get_address(addr=link_name.split("=?")[1].replace("  ", ""))
        try:
            self.overview["mdaas:RegisteredAddress"]["zip"] = self.overview[
                "identifiers"
        ...
        try:
            self.overview["mdaas:RegisteredAddress"]["city"] = self.overview[
                "registeredIn"
            ]
        except:
            pass
