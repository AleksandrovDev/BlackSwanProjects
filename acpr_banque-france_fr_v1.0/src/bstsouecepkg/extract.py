...
class Extract(ABC):
    ...
    def Execute(self, searchquery, fetch_type, action, API_BASE_URL):
        ...
        if self.FETCH_TYPE is None or self.FETCH_TYPE == "":
            ...
            if self.FETCH_TYPE == "graph:shareholders":
                ...
                now = datetime.now()
                job_id = str(
                    hashlib.md5(searchquery.strip().encode("utf-8")).hexdigest()
                ) + str(now.timestamp())
                ...
                data = {
                    "_links": {
                        "shareholders": [
                            {"level": 0, "href": level_0_url},
                            {"level": 1, "href": level_1_url},
                        ]
                    }
                }
    ...
    def __save_graph_in_s3(self, file_name, data):
        ...
        response = self.s3_client.generate_presigned_url(
            "get_object",
            Params={"Bucket": self.DATA_CACHE, "Key": key},
            ...
            try:
                for inner_key, inner_value in main_value.items():
                    if inner_key.lower() == sub_key.lower():
                        name, val = self.get_function_based_on_type(
                            sub_value, inner_value
                        )
...
class GetPages:
    ...
    def getpages_csv(
        ...
        try:
            ...
            df = df[
                df[search_column]
                .str.lower()
                .str.contains(searchquery.lower(), na=False)
            ]
        except Exception as e:
            print(e)
            pass
    ...
    def getpages_excel(
        ...
        try:
            fd = fd = self.get_file(
                xpath, url, headers=headers, data=data, method=method
            )
