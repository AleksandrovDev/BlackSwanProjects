...
class Handler(Extract, GetPages):
    ...
    def getpages(self, searchquery):
        ...
        links = self.get_by_xpath(
            tree, '//div[@class="kategoriContainer"]/a/@href', return_list=True
        )
        categories = self.get_by_xpath(
            tree, '//div[@class="kategoriContainer"]/a/span[2]/text()', return_list=True
        )
        for link in range(len(links)):
            tree = self.get_tree(self.base_url + links[link], headers=self.header)
            names = self.get_by_xpath(
                tree, '//li[@class="row"]/div[1]/text()', return_list=True
            )
            for name in names:
                if searchquery in name:
                    result_links.append(
                        f"{self.base_url + links[link]}?={categories[link]}?={name}"
                    )
        return result_links
    ...
    def get_business_classifier(self, tree, base_xpath, cat):
        final_list = []
        desc = f"{cat}"
        sub_cat = self.get_by_xpath(
            tree, base_xpath + "/../../../../div/h5/button/text()[1]"
        )
        if sub_cat:
            desc += f", {sub_cat}"

        temp_dict = {"code": "", "description": desc, "label": ""}
    ...
    def get_address(self, tree, base_xpath):
        address = self.get_by_xpath(tree, base_xpath + "/div[3]/button/@data-adres")
    ...
    def get_regulator_address(self, tree):
        address = self.get_by_xpath(
            tree, '//td[@class="lead"]/../following-sibling::tr[2]/td/text()'
        )
        temp_dict = {
            "fullAddress": address + ", Turkey",
    ...
    def get_overview(self, link_name):
        ...
        try:
            orga_name = self.get_by_xpath(
                tree, f'//li[@class="row"]/div[1]/text()[contains(., "{name}")]'
            )
        except:
            return None
        if orga_name:
        ...
        self.check_create(tree, base_xpath + "/div[2]/a/@href", "hasURL", company)
        ...
        self.check_create(
            tree,
            base_xpath + "/div[3]/button/@data-aciklama",
            "bst:description",
            company,
        )
        ...
        self.check_create(
            tree,
            base_xpath + "/div[3]/button/@data-telefon",
        ...
        self.check_create(
            tree,
            base_xpath + "/div[3]/button/@data-faks",
