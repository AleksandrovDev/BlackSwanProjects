...
class Handler(Extract, GetPages):
    ...
    def getpages(self, searchquery):
        result = []
        link_1 = "https://www.bankofengland.co.uk/-/media/boe/files/statistics/data-collection/institutions/mfi-list.xlsx?la=en"
        fd = urllib.request.urlopen(link_1)

        try:
            df1 = pd.read_excel(BytesIO(fd.read()), header=None)
            sourceDate = df1.iloc[0, 0]
            fd = urllib.request.urlopen(link_1)
            df = pd.read_excel(BytesIO(fd.read()), header=4)

            for column in range(6):
                search_column = f"Unnamed: {column}"
                df_res = df[
                    df[search_column]
                    .str.lower()
                    .str.replace("\u00a0", " ")
                    .str.contains(searchquery.lower(), na=False, regex=False)
                ]
                array = df_res.iloc[:, column].to_numpy()
                array = [i + "?=" + sourceDate for i in array]
                result.extend(array)
    ...
    def get_overview(self, link_name):
        self.overview = {}
        name = link_name.split("?=")[0].replace("\u00a0", " ")
        sourceDate = link_name.split("?=")[-1]

        try:
            self.overview["vcard:organization-name"] = name
        except:
            return None

        self.overview["isDomiciledIn"] = "GB"
        self.overview["regulator_name"] = "Bank of England"
        self.overview["regulator_url"] = self.base_url
        self.overview["RegulationStatus"] = "Authorised"
        self.overview["regulatorAddress"] = {
            "fullAddress": "Threadneedle Street, London, EC2R 8AH",
            "city": "London",
            "country": "United Kingdom",
        }
        self.fillField(
            "sourceDate",
            el=sourceDate.split("Last updated: ")[-1],
            reformatDate="%d %B %Y",
        )
