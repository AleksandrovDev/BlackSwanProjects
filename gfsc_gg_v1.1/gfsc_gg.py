...
class Handler(Extract, GetPages):
    ...
    def getpages(self, searchquery):
        result = []
        link_1 = "https://www.gfsc.gg/commission/regulated-entities"
        content = self.get_content(link_1)
        x = content.content
        encoding = "utf-8"
        x = (
            x.decode(encoding)
            .split('gfsc_companies":')[-1]
            .split(',"ajaxTrustedUrl"')[0]
        )
        x = json.loads(x)
        companies = x["company-feed"]["companies"]
        all_list = []
        for company in companies:
            if (
                searchquery in company["name"]
                and "Personal" not in company["Filters"]
                and "surrendered" not in company["name"]
            ):
                result.append(
                    "regulated?="
                    + self.base_url
                    + company["name"].split('<a href="')[-1].split('">')[0]
                )
                all_list.append(
                    company["name"].split('<a href="')[-1].split('">')[0].split("/")[-1]
                )

        link_2 = "https://www.gfsc.gg/commission/registered-entities"
        content = self.get_content(link_2)
        x = content.content
        encoding = "utf-8"
        x = (
            x.decode(encoding)
        ...
        for company in companies:
            if searchquery in company["name"] and "surrendered" not in company["name"]:
                res = (
                    company["name"].split('<a href="')[-1].split('">')[0].split("/")[-1]
                )
    ...
    def get_overview(self, link_name):
        ...
        try:
            self.fillField(
                ...
                xpath='//h1[@class="secondary-title"]//text()',
            )
        except:
            return None
        ...
        self.fillField(
            ...
            xpath='//h4/text()[contains(., "Legal Name")]/../following-sibling::p[1]/text()',
        )
        self.overview["isDomiciledIn"] = "GG"
        self.overview["regulator_name"] = "Guernsey Financial Services Commission"
        self.overview["regulator_url"] = "https://www.gfsc.gg/commission"
        self.overview["RegulationStatus"] = "Authorised"
        self.overview["regulatorAddress"] = {
            "fullAddress": "“Glategny Court, Glategny Esplanade,St Peter Port, Guernsey,GY1 1WR",
            "city": "St Peter Port",
            "country": "Guernsey",
        }
        ...
        self.fill_business_classifier(
            xpathDesc='//section[@class="regulated-by"]/section/div//h4/text()',
            xpathLabels='//section[@class="regulated-by"]/section/div//h3/text()',
        )
        ...
        try:
            ...
            self.fill_business_classifier(
                xpathDesc='//section[@class="registered-as"]/section/div//h4/text()',
                xpathLabels='//section[@class="registered-as"]/section/div//h3/text()',
            )

        self.get_address(
            xpath='//div/text()[contains(., "Address")]/../following-sibling::div[1]//text()'
        )
        self.get_operational_address(
            xpath='//h4/text()[contains(., "Principal place of business")]/../following-sibling::text()'
        )
