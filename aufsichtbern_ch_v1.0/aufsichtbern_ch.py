...
class Handler(Extract, GetPages):
    ...
    def getpages(self, searchquery):
        ...
        link_1 = "https://www.aufsichtbern.ch/documents/401772/406362/Liste+des+fondations+BE+du+1er+janvier+2022.pdf"
        link_2 = "https://www.aufsichtbern.ch/documents/401772/406362/Liste+des+fondations+NR+du+1er+janvier+2022.pdf"
        link_3 = "https://www.aufsichtbern.ch/documents/401772/406362/Liste+des+fondations+FR+du+1er+janvier+2022.pdf"
        link_4 = "https://www.aufsichtbern.ch/documents/401772/406362/Liste+des+fondations+FRN+du+1er+janvier+2022.pdf"

        df = self.getpages_pdf(
            pages="all",
            file_base_url=link_1,
        ...
        for page in df:
            addresses = page.iloc[:, 2].str.replace("\r", " ")

            names = page.iloc[:, 1].str.replace("\r", " ")

            registered = page.iloc[:, 5].str.replace("\r", " ")
            postal = page.iloc[:, 3]
            city = page.iloc[:, 4].str.replace("\r", " ")

            for name, addr, cit, pos, reg in zip(
                names, addresses, city, postal, registered
            ):
                if searchquery.lower() in name.lower():
                    result.append(
                        f"{name}=?{addr}=?{cit}=?{pos}=?{reg}=?registered pension funds Canton of Bern=?Registered"
                    )

        df = self.getpages_pdf(
            pages="all",
            file_base_url=link_2,
            searchquery=searchquery,
            search_column=1,
            multiple_tables=True,
        )
        for page in df:
            ...
            postal = page.iloc[:, 3]
            city = page.iloc[:, 4].str.replace("\r", " ")
            types = page.iloc[:, 6].str.replace("\r", " ")

            for name, addr, cit, pos, reg, t in zip(
                names, addresses, city, postal, registered, types
            ):
                if searchquery.lower() in name.lower():
                    result.append(
                        f"{name}=?{addr}=?{cit}=?{pos}=?{reg}=?{t}=?Unregistered"
                    )

        df = self.getpages_pdf(
            pages="all",
            file_base_url=link_3,
            searchquery=searchquery,
            search_column=1,
            multiple_tables=True,
        )
        for page in df:
            ...
            for name, addr, cit, pos, reg in zip(
                ...
                if searchquery.lower() in name.lower():
                    result.append(
                        f"{name}=?{addr}=?{cit}=?{pos}=?{reg}=?Registered pension funds Canton of Friborg=?Registered"
                    )

        df = self.getpages_pdf(
            pages="all",
            file_base_url=link_4,
            searchquery=searchquery,
            search_column=1,
            multiple_tables=True,
        )
    ...
    def get_overview(self, link_name):
        ...
        self.overview["bst:businessClassifier"] = [
            {"code": "", "description": link_name.split("=?")[-2], "label": ""}
        ]

        self.overview["isDomiciledIn"] = "CH"
        self.overview["RegulationStatus"] = link_name.split("=?")[-1]
        if self.overview["RegulationStatus"] == "Registered":
            self.overview["RegulationStatusEffectiveDate"] = self.reformat_date(
                link_name.split("=?")[-3], "%d.%m.%Y"
            )

        else:
            self.overview["isIncorporatedIn"] = self.reformat_date(
                link_name.split("=?")[-3], "%d.%m.%Y"
            )

        self.overview["mdaas:RegisteredAddress"] = {
            "streetAddress": link_name.split("=?")[1],
            "country": "Switzerland",
        }
        try:
            self.overview["mdaas:RegisteredAddress"]["zip"] = link_name.split("=?")[3]
            self.overview["mdaas:RegisteredAddress"]["city"] = link_name.split("=?")[2]
