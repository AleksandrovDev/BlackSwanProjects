...
class Handler(Extract, GetPages):
    ...
    def getpages(self, searchquery):
        ...
        companies = self.get_result_list_by_path(
            '//div[@class="region region-content"]//table//tr/td[1]//text()'
        )
        return companies

    def get_initial_page(self, searchquery):
        link = f"http://www.me.bf/en/annonces-legales?keys={searchquery}"
        requestOptions = {
            "url": link,
            "method": "GET",
        }
    ...
    def get_overview(self, link):
        ...
        path = f'//div[@class="region region-content"]//table//tr/td[1]//text()[contains(., "{link}")]/../..'
    ...
    def fill_field(self, fieldName, data=None, reformatDate=None, el=None):
        ...
        if fieldName in self.complicatedFields:
            self.fill_dictionary_field(
                dataPath, handlingFunctions, fieldName, returnType, showEmpty, dataType
            )
    ...
    def get_address_from_entity(self, entityAddress):
        try:
            temp = {
                ...
                "country": self.get_country_name_by_iso_code(
                    entityAddress["country"][0]
                ),
    ...
    def get_path_type(self, dataPath):
        ...
        if type(dataPath) == dict:
            ...
            dataPath = [i for i in dataPath if i != ""][0]
        if "/" in dataPath[:2]:
            return "tree"
    ...
    def fill_dictionary_field(
        ...
        for path in paths:
            if path:
                ...
                codes = self.getCleanValues(codes)
